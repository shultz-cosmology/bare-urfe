...
We perform this by taking the **partial trace** over the sequence subsystem (removing the $|t_n\rangle$ baseline). We project the pure 4D state $|\Omega\rangle$ into a 3D observable density matrix $\rho_{3D}$:

$$\rho_{3D} = \text{Tr}_{\text{time}}\Big(|\Omega\rangle\langle\Omega|\Big) = \sum_{k=1}^{N} \langle t_k | \Omega \rangle \langle \Omega | t_k \rangle$$
